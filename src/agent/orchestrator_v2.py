...
重要优先规则：
1. **优先使用 Skill**：当任务可以被 Skill 覆盖时，优先选择 "skill:<name>" 而非 "builtin-<name>"。Skill 是经过验证的最佳实践提示模板。
2. **内置 Agent 兜底**：只有当任务需要特定工具执行（读文件、写文件、代码搜索）且 Skill 无法覆盖时，才使用 "builtin-*" agent。
3. 每个步骤的描述必须明确说明该步骤的输入和输出
